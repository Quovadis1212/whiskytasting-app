...
export default function Rate({ tasting, setTasting, participant, setParticipant, admin }) {
  ...
  // Drams nach Ausschankreihenfolge
      ...
      <Card className="mb-3 shadow-sm">
        <Card.Body>
          <div className="d-flex justify-content-between align-items-center mb-2">
            ...
            <div className="fs-4">{getR(current.order).points}</div>
          </div>
          <Form.Label className="small text-muted d-flex justify-content-between">
            <span>0</span><span>50</span><span>100</span>
          </Form.Label>
          <Form.Range min={0} max={100}
            value={getR(current.order).points}
            onChange={(e)=>setR(current.order, { points: parseInt(e.target.value,10) })}
          />
        </Card.Body>
      </Card>
      ...
      <Card className="mb-3 shadow-sm">
        <Card.Body>
          ...
          <ButtonGroup className="flex-wrap">
            {AROMAS.map(a => {
              const active = (getR(current.order).aromas || []).includes(a);
              return (
                <Button key={a} size="sm"
                  variant={active ? "warning" : "outline-warning"}
                  className="me-2 mb-2"
                  onClick={()=>toggleAroma(current.order, a)}
                >
                  {a}
                </Button>
              );
      ...
      <Card className="mb-3 shadow-sm">
        <Card.Body>
          ...
          <Form.Control as="textarea" rows={4}
            ...
            value={getR(current.order).notes}
            onChange={(e)=>setR(current.order, { notes: e.target.value })}
          />
        </Card.Body>
      </Card>

      {/* Sticky Action Bar: Speichern + Zur Auflösung */}
